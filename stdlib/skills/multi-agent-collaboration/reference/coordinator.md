...
- **技能与自身** — `prismagent_skill_dir_get`、`prismagent_self_show`、
...
注入的内容包含：
- Workflow UUID 和标题
- Machine-readable YAML block（优先使用）
- 控制流描述（Mermaid 图）
...
Coordinator 必须按以下顺序理解 Workflow：

1. **优先解析 fenced `yaml` 块**：读取 `workflow`、`planner_outputs`、`contexts`、`agents`、`triggers`、`advance`、`completion`、`failure_handling`。
2. **再参考 Mermaid**：用于快速理解控制流形状，尤其是串行、并行、汇合和条件分支。
3. **最后参考文字推进指导**：用于处理 YAML 难以表达的异常策略或人工说明。

如果 YAML 与 Mermaid/文字冲突：
- UUID、agent、context、trigger、context_refs、context_out 以 YAML 为准；
- 条件分支与异常策略优先参考文字说明；
- 冲突会导致工作流不可安全推进时，通知 Planner 重建 workflow。

YAML 字段约定：
- `agents[].depends_on_contexts`：创建该 agent 前必须存在的 context。
- `triggers[].watch_contexts`：创建 trigger 时传给 `prismagent_trigger_create.context_uuids`；运行时是 OR 语义。
- `advance.steps[].when_all_contexts_exist`：Coordinator 用它表达 AND 汇合，只有全部 context 都存在才推进。
- `workflow.final_contexts`：完成时应发送给 Planner 的最终 context 候选。
...
### 1. 更新Planner的context_out

优先解析 Workflow YAML 的 `planner_outputs.context_uuids`，获取 Planner 应该创建的 Context UUID 列表，使用 `agent_update` 更新 Planner 的 context_out。

#### 从Workflow中获取任务说明Context UUID列表
Planner Context_uuids = ["ctx-task", "ctx-1", "ctx-2"]

#### 更新Planner的context_out
prismagent_agent_update(
  agent_uuid="planner-uuid",
  context_out=[planner_context_uuids]
)

### 2. 检查Context是否实际存在
使用prismagent_agent_list检查Planner的context_out中的Context是否都已创建。
如果缺失，说明Planner没有正确创建Context，通知Planner重新创建。

### 3. 检查工作流中UUID是否合理且不冲突
例如，ctx-1 不能既是Planner的context_out又是executor的context_out，这会导致UUID冲突和工作流不可行。
如果发现UUID冲突，通知Planner重新生成UUID并重建工作流。
但是ctx-1可以是executor的context_out，同时也是另一个executor/verifier的context_refs，这表明了上下游依赖关系，这是合理的。

### 4. 若检查通过，则可以继续后续步骤；若检查未通过，通知Planner修正后重新提交Workflow。
...
```json
// Workflow YAML 中的 triggers
{
  "trigger-1-uuid": {
    "watch_contexts": ["ctx-staging-result-uuid"],
    "message": "executor-1 完成了暂存区分析"
  },
  "trigger-2-uuid": {
    "watch_contexts": ["ctx-log-result-uuid"],
...
prismagent_trigger_create(
  ...
  context_uuids=["ctx-staging-result-uuid"],  # from trigger.watch_contexts
  message="executor-1 完成了暂存区分析"
)
```
